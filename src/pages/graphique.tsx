...
const Graphique = () => {
    ...
    const formatItem = (item: any) => ({
        ...item,
        timeLabel: item.display_time || (item.time ? item.time.split('T')[1] : '--:--'),
        temp: item["temperature_2m (°C)"],
        humi: item["relative_humidity_2m (%)"],
        wind: item["wind_speed_10m (km/h)"],
        clouds: item["cloud_cover (%)"] || 0,
    });

    const fetchData = async () => {
        try {
            const response = await fetch(`http://localhost:8000/api/weather/history?limit=${maxPoints}`);
            const result = await response.json();
            const formatted = result.reverse().map((item: any) => formatItem(item));
            setData(formatted);
        } catch (error) {
    ...
    useEffect(() => {
        ...
        ws.onmessage = (event) => {
            ...
            setData((prev) => [...prev, formatItem(newItem)].slice(-maxPoints));
        };
        return () => ws.close();
    }, []);

    const pieData = useMemo(() => {
        const sunny = data.filter(d => d.clouds < 20).length;
        const cloudy = data.filter(d => d.clouds >= 20 && d.clouds < 70).length;
        const overcast = data.filter(d => d.clouds >= 70).length;
        return [
    ...
    if (loading) return <div className="p-20 text-center font-bold animate-pulse">Synchronisation des flux
        live...</div>;
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-gray-400">Mesure de la température de l'air à 2 mètres du
                                    sol</p>
                            </div>
                        </div>
                        <div className="h-[300px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={data}>
                                    ...
                                    <XAxis dataKey="timeLabel" fontSize={10} tickLine={false} axisLine={false}
                                           interval={5}/>
                        ...
                        <div className="mt-6 p-4 bg-gray-50 rounded-xl">
                            <p className="text-sm text-gray-600 leading-relaxed">
                                <span className="font-bold text-blue-600">Analyse :</span> Ce graphique illustre la
                                tendance de réchauffement ou de refroidissement.
                                Une courbe stable indique un équilibre thermique, tandis que des pics soudains peuvent
                                signaler un ensoleillement direct sur la station.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 flex flex-col">
                        ...
                        <p className="text-[11px] text-gray-400 mt-4 bg-amber-50 p-3 rounded-lg border border-amber-100 text-center italic">
                            Répartition calculée sur la base du pourcentage de nébulosité des 50 dernières minutes.
                        </p>
                    </div>
                    ...
                    <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                        ...
                        <p className="mt-4 text-[13px] text-gray-500">
                            Affiche les changements brusques de vitesse. Le mode "Escalier" (Step) est utilisé pour
                            mieux visualiser les rafales instantanées.
                        </p>
                    </div>
                    ...
                    <div className="lg:col-span-2 bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                        ...
                        <div className="h-[200px]">
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={data}>
                                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9"/>
                                    <XAxis dataKey="timeLabel" fontSize={10} axisLine={false} tickLine={false}
                                           interval={10}/>
                        ...
                        <p className="mt-4 text-[13px] text-gray-500 italic">
                            Plus le taux est élevé, plus l'air est chargé en humidité. Un taux supérieur à 80% est
                            souvent précurseur de précipitations ou de brouillard.
                        </p>
                    </div>
